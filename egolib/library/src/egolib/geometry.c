...
bool two_plane_intersection(Ego::Vector3f& p, Ego::Vector3f& d, const Ego::Plane3f& p0, const Ego::Plane3f& p1)
{
    // Compute \f$\vec{d} = \hat{n}_0 \times \hat{n}_1\f$
    const Ego::Vector3f &n0 = p0.get_normal();
    const Ego::Vector3f &n1 = p1.get_normal();
    d = Ego::cross(n0, n1);
    
    // If \f$\vec{v}\f$ is the zero vector, then the planes do not intersect.
    if (idlib::zero<Ego::Vector3f>() == d) {
        return false;
    }
    d = Ego::normalize(d).get_vector();
    if (0.0f != d[kZ])
    {
...
Ego::Math::Relation cone_intersects_point(const Ego::Cone3f& K, const Ego::Point3f& P)
{
    ...
    const auto rhs = idlib::euclidean_norm(t) * std::cos(K.get_angle());
    // Compute \f$\hat{d} \cdot \left(t\right)\f$.
    const auto lhs = idlib::dot_product(K.get_axis(), t);
    if (lhs > rhs) {
        return Ego::Math::Relation::inside;
    } else if (lhs < rhs) {
        return Ego::Math::Relation::outside;
...
bool sphere_intersects_cone(const Ego::Sphere3f& S, const Ego::Cone3f& K) {
    ...
    Ego::Vector3f t; float lhs, rhs;

    // Determine the relation of the center to the forward cone.
    t = S.get_center() - origin_p;
    rhs = idlib::euclidean_norm(t) * c;
    lhs = Ego::dot(direction_p, t);
    if (lhs < rhs) {
    ...
    rhs = idlib::euclidean_norm(t) * s; // \f$\cos\left(90-\theta\right) = \sin(\theta)\f$
    lhs = Ego::dot(direction_m, t);
    if (lhs < rhs) {
        ...
        float l2 = idlib::squared_euclidean_norm(t), r2 = S.get_radius() * S.get_radius();
        return l2 <= r2;
    } else {

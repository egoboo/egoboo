...
EgoTest_TestCase(LinearInterpolation) {
public:
    template <typename T>
...
public:
    ...
    // Add red, green, blue.
    list<Colour3f> rgb(list<Colour3f> l) {
        l.push_back(Colour3f::red());
        l.push_back(Colour3f::green());
        l.push_back(Colour3f::blue());
        return l;
    }

    static void test(const Colour3f& x, const Colour3f& y) {
        auto f = Math::Lerp<Colour3f>();
...
public:
    ...
    // Add opaque red, green, blue.
    list<Colour4f> rgba(list<Colour4f> l) {
        l.push_back(Colour4f::red());
        l.push_back(Colour4f::green());
        l.push_back(Colour4f::blue());
        return l;
    }

    static void test(const Colour4f& x, const Colour4f& y) {
        auto f = Math::Lerp<Colour4f>();
        EgoTest_Assert(x == f(x, y, 0.0f));
        EgoTest_Assert(y == f(x, y, 1.0f));
    }

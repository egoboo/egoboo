...
bool_t input_settings_save_vfs_3( const char* szFilename )
{
    ...
    for ( i = 0; i < MAX_LOCAL_PLAYERS; i++ )
    {
        ...
        export_control( filewrite, "Left Hand Get/Drop   ", pdevice->device_type, pdevice->control + CONTROL_LEFT_PACK );
        export_control( filewrite, "Left Hand Inventory  ", pdevice->device_type, pdevice->control + CONTROL_INVENTORY );

...
{
    /// @author ZZ
    /// @details This function loads a particle template, returning false if the file wasn't
    ///    found
    ...
    if ( 'L' == char_toupper(( unsigned )cTmp ) )  ppip->type = SPRITE_LIGHT;
    else if ( 'S' == char_toupper(( unsigned )cTmp ) )  ppip->type = SPRITE_SOLID;
    else if ( 'T' == char_toupper(( unsigned )cTmp ) )  ppip->type = SPRITE_ALPHA;
    ...
    // Lighting data
    cTmp = vfs_get_next_char( fileread );
    if ( 'T' == char_toupper(( unsigned )cTmp ) ) ppip->dynalight.mode = DYNA_MODE_ON;
    else if ( 'L' == char_toupper(( unsigned )cTmp ) ) ppip->dynalight.mode = DYNA_MODE_LOCAL;

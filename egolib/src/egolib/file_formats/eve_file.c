...
eve_t * load_one_enchant_file_vfs( const char* szLoadName, eve_t * peve )
{
    ...
    if ( 'R' == char_toupper(( unsigned )cTmp ) )       peve->setvalue[SETMISSILETREATMENT] = MISSILE_REFLECT;
    else if ( 'D' == char_toupper(( unsigned )cTmp ) )  peve->setvalue[SETMISSILETREATMENT] = MISSILE_DEFLECT;
    ...
    peve->setvalue[SETMORPH] = true;  // vfs_get_bool( fileread );        //ZF> huh? why always channel and morph?

    peve->setyesno[SETCHANNEL] = vfs_get_next_bool( fileread );
    peve->setvalue[SETCHANNEL] = true;  // vfs_get_bool( fileread );

...
/// @tparam ElementType the element type. Must fulfil the ordered integral domain concept.
/// @tparam Dimensionality the dimensionality. Must fulfil the dimensionality concept.
template <typename Element, size_t Dimensionality>
struct tuple {
public:
    ...
    constexpr static size_t dimensionality() noexcept
    {
        return Dimensionality;
    }

    /// @invariant @a Dimensionality must fulfil the <em>dimensionality</em> concept.
    static_assert(id::is_dimensionality_v<Dimensionality>, "Dimensionality must fulfil the dimensionality concept");

	/// @brief @a this_type is the type of this template/template specialization.
	using this_type = tuple<Element, Dimensionality>;

private:
	/// @brief The elements of this tuple.
	std::array<Element, Dimensionality> m_elements;
...
protected:
    ...
    tuple(const Generator& generator, std::index_sequence<Index ...>)
	...
	template<
        ...
        typename = 
            std::enable_if_t<
                (1 + sizeof...(ArgumentTypes)) == this_type::dimensionality() &&
                Core::AllConvertible<Element, ArgumentTypes ...>::value
            >
    >
	tuple(Element&& first, ArgumentTypes&& ... rest) :
        m_elements{ static_cast<Element>(first), static_cast<Element>(rest) ... }
    {
...
protected:
	/// @brief Default-construct this tuple.
	tuple() :
        m_elements()
    {}
...
protected:
	/// @brief Copy-construct this tuple with the values of another tuple.
...
protected:
	/// @brief Assign this tuple with the values of another tuple.
	/// @param other the other tuple
...
public:
    ...
    /// @name "minimal/maximal element value"
    /// @brief Determination of minimal and maximal value of a tuple.
    /// @{
    ...
    {
        return *std::min_element(m_elements.cbegin(), m_elements.cend());
    }
    ...
    {
        return *std::max_element(m_elements.cbegin(), m_elements.cend());
    }

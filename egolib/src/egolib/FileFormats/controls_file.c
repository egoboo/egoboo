...
bool input_settings_load_vfs(const char *szFilename, int required_version)
{
    ...
    {
		Log::get().warn( "File version (%d) did match the required version (%d)\n", file_version, required_version );
    ...
    switch ( file_version )
    {
        ...
        default:
			Log::get().warn( "Cannot load the given controls.txt file because required version %d is not supported.\n", file_version );
            retval = false;
            break;
    }
...
bool input_settings_save_vfs(const char* szFilename, int version)
{
    ...
    switch ( version )
    {
        ...
        default:
			Log::get().warn( "Cannot save the given setting.txt file because version %d is not supported.\n", version );
            retval = false;
            break;
    }

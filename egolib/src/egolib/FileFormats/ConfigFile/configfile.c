...
ConfigEntry::ConfigEntry(const idlib::c::qualified_name& qualifiedName, const std::string& value) :
...
const idlib::c::qualified_name& ConfigEntry::getQualifiedName() const
{
...
bool ConfigFileParser::parseQualifiedName()
{
    ...
    _currentQualifiedName.reset(new idlib::c::qualified_name(get_lexeme_text()));
    ...
    try
    {
        ...
        std::sort(entries.begin(), entries.end(),
             [] (const std::shared_ptr<ConfigEntry>& a, const std::shared_ptr<ConfigEntry>& b)
                {
                    return std::less<idlib::c::qualified_name>()(a->getQualifiedName(), b->getQualifiedName());
                }
            );

...
struct egonet_instance_t
{
    ...
    bool enet_on;                 ///< enet is active
    bool service_on;              ///< Do I need to free the interface?
    bool hostactive;              ///< Hosting?
    bool readytostart;            ///< Ready to hit the Start Game button?
    bool waitingforclients;       ///< Has everyone talked to the host?
...
#define EGONET_INSTANCE_INIT \
    { \
        ...
        false,    /* bool req_enet       */ \
        false,    /* bool enet_on      */ \
        false,    /* bool service_on    */ \
        false,    /* bool hostactive   */ \
        false,    /* bool readytostart */ \
        false,    /* bool waitingforclients */ \
        -1,       /* int     service */ \
        0,        /* int     service_count */ \
...
//--------------------------------------------------------------------------------------------
const bool egonet_get_serviceon()
{
    if ( !gnet.initialized ) return false;
...
//--------------------------------------------------------------------------------------------
const ENetHost * egonet_get_myHost()
{
    if ( !gnet.initialized ) return NULL;
...
bool ego_packet_addUint8( ego_packet_t * ptr, Uint8 uc )
{
    ...
    if ( NULL == ptr || ptr->size >= sizeof( ptr->buffer ) ) return false;
...
//--------------------------------------------------------------------------------------------
bool egonet_broadcastPacket(ego_packet_t * ptr)
{
    /// @author ZZ
    ...
    // valid data?
    if ( NULL == ptr || 0 == ptr->size ) return false;
...
//--------------------------------------------------------------------------------------------
bool egonet_broadcastPacketGuaranteed(ego_packet_t * ptr)
{
    /// @author ZZ
    /// @details This function sends a packet to all the players
...
//--------------------------------------------------------------------------------------------
bool egonet_sendPacketToOneClientGuaranteed(ego_packet_t * ptr, int player)
{
    /// @author ZZ
    ...
    // valid player?
    if ( player >= gnet.pserver->client_count ) return false;
...
//--------------------------------------------------------------------------------------------
bool enet_packet_startReading(enet_packet_t * pkt, ENetPacket *packet)
{
...
//--------------------------------------------------------------------------------------------
bool enet_packet_doneReading(enet_packet_t * pkt)
{
    if ( NULL == pkt ) return false;
...
//--------------------------------------------------------------------------------------------
bool enet_packet_readString(enet_packet_t * pkt, char *buffer, size_t maxLen)
{
    /// @author ZZ
    ...
    // valid packet?
    if ( NULL == pkt || NULL == pkt->ptr ) return false;

    // read past the end?
    if ( pkt->read_location >= pkt->ptr->dataLength ) return false;
...
//--------------------------------------------------------------------------------------------
bool enet_packet_readUint8(enet_packet_t * pkt, Uint8 * pval)
{
    /// @author ZZ
...
//--------------------------------------------------------------------------------------------
bool enet_packet_readSint8(enet_packet_t * pkt, Sint8 * pval)
{
    /// @author ZZ
...
//--------------------------------------------------------------------------------------------
bool enet_packet_readUint16(enet_packet_t * pkt, Uint16 * pval)
{
    /// @author ZZ
...
//--------------------------------------------------------------------------------------------
bool enet_packet_readSint16(enet_packet_t * pkt, Sint16 * pval)
{
    /// @author ZZ
...
//--------------------------------------------------------------------------------------------
bool enet_packet_readUint32(enet_packet_t * pkt, Uint32 * pval)
{
    /// @author ZZ
...
//--------------------------------------------------------------------------------------------
bool enet_packet_readSint32(enet_packet_t * pkt, Sint32 * pval)
{
    /// @author ZZ
...
{
    if ( NULL == pnet || !pnet->initialized ) return false;
...
//--------------------------------------------------------------------------------------------
bool egonet_instance_set_myHost(egonet_instance_t * pnet, const ENetHost* val)
{
    if ( NULL == pnet || !pnet->initialized ) return false;

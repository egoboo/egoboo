...
bool chr_calc_grip_cv( Object * pmount, int grip_offset, oct_bb_t * grip_cv_ptr, const bool shift_origin )
{
    ...
    // get appropriate vertices for this model's grip
    {
        ...
        else if ( grip_count > 0 )
        {
            ...
            if ( grip_count < 2 )
            {
                grip_points[2] = idlib::zero<Vector4f>();
                grip_points[2][kY] = 1.0f;
            }

            if ( grip_count < 3 )
            {
                grip_points[3] = idlib::zero<Vector4f>();
        ...
        else if ( 0 == grip_count )
        {
            ...
            for ( cnt = 0; cnt < 4; cnt++ )
            {
                grip_points[cnt] = idlib::zero<Vector4f>();
            }
    ...
    if ( NULL != grip_cv_ptr )
    {
        *grip_cv_ptr = idlib::translate(tmp_cv, Vector3f(grip_nupoints[0][kX], grip_nupoints[0][kY], grip_nupoints[0][kZ]));
    }

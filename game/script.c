...
unsigned char           cLineBuffer[MAXLINESIZE];
int                     iLoadSize;
int                     iLineSize;
int                     iNumLine;
unsigned char           cCodeType[MAXCODE];
unsigned int            iCodeValue[MAXCODE];
unsigned char           cCodeName[MAXCODE][MAXCODENAMESIZE];
int                     iCodeIndex;
int                     iCodeValueTmp;
int                     iNumCode;
unsigned int            iCompiledAis[AISMAXCOMPILESIZE];
...
int load_ai_script(char *loadname)
{
	...
	if(fileread == NULL) 
	{
		log_message("NOTE: I am missing a AI script (%s)\n", loadname);
		log_message("Using the default AI script instead (basicdat/script.txt)\n");
	}
...
unsigned char run_function(unsigned int value, int character)
{
	...
	unsigned char returncode = btrue;
    ...
    switch(valuecode)
    {
        ...
        case FGOPOOF:
            ...
            returncode = bfalse;
            if(chrisplayer[character]==bfalse)
            {
                returncode = btrue;
                valuegopoof = btrue;
            }
        ...
        case FIFTARGETISONHATEDTEAM:
            ...
            returncode = (chralive[chraitarget[character]] && teamhatesteam[chrteam[character]][chrteam[chraitarget[character]]] && chrinvictus[chraitarget[character]] == bfalse );
            break;
        ...
        case FIFTARGETISHURT:
            ...
            if(chralive[chraitarget[character]]==bfalse || chrlife[chraitarget[character]] > chrlifemax[chraitarget[character]]-HURTDAMAGE)
                returncode = bfalse;
            break;
        ...
        case FIFTARGETISALIVE:
            ...
            returncode = (chralive[chraitarget[character]]==btrue);
            break;
        case FSTOP:
        ...
        case FIFHOLDINGITEMID:
            ...
            if(sTmp != MAXCHR)
            {
                ...
                {
                    valuetmpargument = LATCHBUTTONRIGHT;
                    if(returncode == btrue)  valuetmpargument = LATCHBUTTONLEFT+(rand()&1);
                    returncode = btrue;
                }
            }
            break;
        ...
        case FIFHOLDINGMELEEWEAPON:
            ...
            {
                sTmp = chrmodel[sTmp];
                if(capisranged[sTmp]==bfalse && capweaponaction[sTmp]!=ACTIONPA)
                {
                    valuetmpargument = LATCHBUTTONLEFT;
                    returncode = btrue;
                }
...
//            chricon[character] = btrue;
        ...
        case FPOOFTARGET:
            ...
            returncode = bfalse;
            if(chrisplayer[chraitarget[character]]==bfalse)
            {
                returncode = btrue;
		...
		case FIFEQUIPPED:
			...
			returncode = bfalse;
			if(chrisequipped[character] == btrue) returncode = btrue;
            break;
        ...
        case FIFTARGETISAPLATFORM:
            ...
            break;
        case FADDSTAT:
            if(chrstaton[character]==bfalse) add_stat(character);
            break;
...
*/
        ...
        case FIFTARGETHASNOTFULLMANA:
            ...
            if(chralive[chraitarget[character]]==bfalse || chrmana[chraitarget[character]] > chrmanamax[chraitarget[character]]-HURTDAMAGE)
                returncode = bfalse;
            break;
		...
		default: returncode = btrue; break;
...
void let_character_think(int character)
{
    ...
    // Set latches
    if(chrisplayer[character]==bfalse && aicode!=0)
    {

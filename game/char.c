            ...
            else
            {
                ...
                if(chrtransferblend[character]&&chrisitem[cnt])
                {
					...
					if(chrisplayer[character] && chrlight[character] != 255)
						chrlight[cnt] = 128;
					else chrlight[cnt] = chrlight[character];
            ...
            if(chralive[cnt])
            {
                ...
                if(chrlatchbutton[cnt]!=0)
                {
                    ...
                    if(chrlatchbutton[cnt]&LATCHBUTTONLEFT&&chrreloadtime[cnt]==0)
                    {
						...
						if(madactionvalid[chrmodel[cnt]][action]==bfalse || chrreloadtime[weapon] > 0) allowedtoattack = bfalse;
						else
						{
...
void export_one_character_profile(char *szSaveName, unsigned short character)
{
    ...
    if(filewrite)
    {
        ...
        if(capcanuseadvancedweapons[profile] == ('A'-'A'<<15)|('W'-'A'<<10)|('E'-'A'<<5)|('P'-'A'))
...
*/
        ...
        while(goto_colon_yesno(fileread))
        {
			...
			test = ('A'-'A'<<15)|('W'-'A'<<10)|('E'-'A'<<5)|('P'-'A');  // [AWEP]
            if(idsz == test)  capcanuseadvancedweapons[object] = test;
			test = ('S'-'A'<<15)|('H'-'A'<<10)|('P'-'A'<<5)|('R'-'A');  // [SHPR]

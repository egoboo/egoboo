...
static INLINE bool_t ego_rpc_finished( const ego_rpc_base_t * prpc, const int guid ) { return !ego_rpc_matches( prpc, guid ) || ( prpc->finished ); }
static INLINE bool_t ego_rpc_abort( ego_rpc_base_t * prpc, const int guid ) { if ( !ego_rpc_matches( prpc, guid ) ) return bfalse; prpc->abort = btrue; return btrue; }

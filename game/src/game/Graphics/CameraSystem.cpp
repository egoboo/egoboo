...
void CameraSystem::endCameraMode()
{
    ...
    Ego::Renderer::get().setViewportRectangle(0, 0, sdl_scr.width, sdl_scr.height);
...
void CameraSystem::beginCameraMode( const std::shared_ptr<Camera> &camera)
{
    ...
    renderer.setScissorRectangle(camera->getScreen().xmin, sdl_scr.height - camera->getScreen().ymax, camera->getScreen().xmax - camera->getScreen().xmin, camera->getScreen().ymax - camera->getScreen().ymin);

    // set the viewport
    renderer.setViewportRectangle(camera->getScreen().xmin, sdl_scr.height - camera->getScreen().ymax, camera->getScreen().xmax - camera->getScreen().xmin, camera->getScreen().ymax - camera->getScreen().ymin);
}
...
void CameraSystem::autoFormatTargets()
{
    ...
    float aspect_ratio = static_cast<float>(sdl_scr.width) / static_cast<float>(sdl_scr.height);
    ...
    if ( widescreen )
    {
        switch ( _cameraList.size() )
        {
            ...
            case 1:
                // fullscreen
                _cameraList[0]->setScreen(0.0f, 0.0f, sdl_scr.width, sdl_scr.height);
                break;

            case 2:
                // wider than tall, so windows are side-by side
                _cameraList[0]->setScreen(0.0f, 0.0f, sdl_scr.width * 0.5f - border, sdl_scr.height);
                _cameraList[1]->setScreen(sdl_scr.width * 0.5f + border, 0.0f, sdl_scr.width, sdl_scr.height);
                break;

            case 3:
                // wider than tall, so windows are side-by side
                _cameraList[0]->setScreen(0.0f, 0.0f, sdl_scr.width / 3.0f - border, sdl_scr.height);
                _cameraList[1]->setScreen(sdl_scr.width / 3.0f + border, 0.0f, 2.0f * sdl_scr.width / 3.0f - border, sdl_scr.height);
                _cameraList[2]->setScreen(2.0f * sdl_scr.width / 3.0f + border, 0.0f, sdl_scr.width, sdl_scr.height);
                break;

            case 4:
                // 4 panes
                _cameraList[0]->setScreen(0.0f, 0.0f, sdl_scr.width * 0.5f - border, sdl_scr.height * 0.5f - border);
                _cameraList[1]->setScreen(sdl_scr.width * 0.5f + border, 0.0f, sdl_scr.width, sdl_scr.height * 0.5f - border);
                _cameraList[2]->setScreen(0.0f, sdl_scr.height * 0.5f + border, sdl_scr.width * 0.5f - border, sdl_scr.height);
                _cameraList[3]->setScreen(sdl_scr.width * 0.5f + border, sdl_scr.height * 0.5f + border, sdl_scr.width, sdl_scr.height);
                break;
        }
    }
    else
    {
        switch ( _cameraList.size() )
        {
            ...
            case 2:
                if ( sdl_scr.width  >= sdl_scr.height )
                {
                    // wider than tall, so windows are side-by side
                    _cameraList[0]->setScreen(0.0f, 0.0f, sdl_scr.width * 0.5f - border, sdl_scr.height);
                    _cameraList[1]->setScreen(sdl_scr.width * 0.5f + border, 0.0f, sdl_scr.width, sdl_scr.height);
                ...
                else
                {
                    ...
                    _cameraList[0]->setScreen(0.0f, 0.0f, sdl_scr.width, sdl_scr.height * 0.5f - border);
                    _cameraList[1]->setScreen(0.0f, sdl_scr.height * 0.5f + border, sdl_scr.width, sdl_scr.height);
                }
                break;
            ...
            case 3:
            	...
            	_cameraList[0]->setScreen(0.0f, 0.0f, sdl_scr.width * 0.5f - border, sdl_scr.height * 0.5f - border);
            	_cameraList[1]->setScreen(sdl_scr.width * 0.5f + border, 0.0f, sdl_scr.width, sdl_scr.height * 0.5f - border);
            	_cameraList[2]->setScreen(0.0f, sdl_scr.height * 0.5f + border, sdl_scr.width, sdl_scr.height);

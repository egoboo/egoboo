...
{
    _mProjection = Matrix4f4f::perspective(fov_deg, aspect_ratio, frustum_near, frustum_far);
...
void Camera::resetView()
{
    ...
    if (_pos != _center)
    {
		...
		Matrix4f4f tmp = Matrix4f4f::scaling(Vector3f(-1.0f, 1.0f, 1.0f)) * Matrix4f4f::rotation(Z, roll_deg);
		_mView = tmp * Matrix4f4f::lookAt(_pos, _center, Z);
    }
    // Invalidate the frustum.
    _frustumInvalid = true;
}

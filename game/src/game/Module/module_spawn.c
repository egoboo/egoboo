...
std::shared_ptr<Object> activate_spawn_file_spawn(spawn_file_info_t& psp_info, const std::shared_ptr<Object> &parent)
{
    ...
    // Spawn the character
    std::shared_ptr<Object> pobject = _currentModule->spawnObject(psp_info.pos, iprofile, psp_info.team, psp_info.skin, Facing(psp_info.facing), psp_info.pname == nullptr ? "" : psp_info.pname, ObjectRef::Invalid);

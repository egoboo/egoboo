...
prt_t *prt_t::config_do_init()
{
    ...
    pprt->is_homing = ppip->homing && !pprt->isAttached();
...
prt_bundle_t *prt_bundle_t::move_one_particle_do_homing()
{
	...
	if (loc_pprt->isAttached() || !_currentModule->getObjectHandler().exists(loc_pprt->target_ref)) return this;
...
prt_bundle_t *prt_bundle_t::updateParticleSimpleGravity()
{
    ...
    if (!this->_prt_ptr->no_gravity && this->_prt_ptr->type == SPRITE_SOLID && !this->_prt_ptr->is_homing && !this->_prt_ptr->isAttached())
    {
...
prt_bundle_t *prt_bundle_t::move_one_particle_do_z_motion()
{
    ...
    if (loc_pprt->no_gravity || /* loc_pprt->type == SPRITE_LIGHT || */ loc_pprt->is_homing || loc_pprt->isAttached()) return this;
...
prt_bundle_t *prt_bundle_t::update_do_water()
{
    ...
    if (inwater && water._is_water && this->_pip_ptr->end_water)
    {
        ...
        if (this->_prt_ptr->isAttached() && this->_prt_ptr->owner_ref == this->_prt_ptr->attachedto_ref)
        {
    ...
    else if (inwater)
    {
        ...
        else
        {
            ...
            {
                if (SPRITE_SOLID == this->_prt_ptr->type && !this->_prt_ptr->isAttached())
                {
...
bool prt_t::isAttached() const
{
    return _currentModule->getObjectHandler().exists(attachedto_ref);
}

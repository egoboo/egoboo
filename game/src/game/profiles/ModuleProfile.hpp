...
public:
	...
	bool isStarterModule() const {return _importAmount == 0;}

	const uint8_t getRank() const {return _rank;}
	...
	**/
	...
	const std::string& getFolderName() const {return _folderName;}

	uint8_t getImportAmount() const {return _importAmount;}

	uint8_t getMinPlayers() const {return _minPlayers;}

	uint8_t getMaxPlayers() const {return _maxPlayers;}

	bool isExportAllowed() const {return _allowExport;}

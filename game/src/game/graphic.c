...
Ego::Texture *TextureAtlasManager::sml[MESH_IMG_COUNT];
Ego::Texture *TextureAtlasManager::big[MESH_IMG_COUNT];
...
Ego::Texture *TextureAtlasManager::get_sml(int which)
{
    if (!initialized)
    {
        return nullptr;
...
Ego::Texture *TextureAtlasManager::get_big(int which)
{
    if (!initialized)
    {
        return nullptr;
...
int TextureAtlasManager::decimate_one_mesh_texture(const Ego::Texture *src_tx, Ego::Texture *(&tx_lst)[MESH_IMG_COUNT], size_t tx_lst_cnt, int minification)
{
    ...
    for (iy = 0, fy = 0.0f; iy < SUB_TEXTURES; iy++, fy += step_fy)
    {
        ...
        for (ix = 0, fx = 0.0f; ix < SUB_TEXTURES; ix++, fx += step_fx)
        {
            ...
            Ego::Texture *dst_tx = new Ego::OpenGL::Texture();

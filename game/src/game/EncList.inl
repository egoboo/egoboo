...
static INLINE bool _VALID_ENC_RANGE( const ENC_REF IENC );
static INLINE bool _DEFINED_ENC( const ENC_REF IENC );
static INLINE bool _ALLOCATED_ENC( const ENC_REF IENC );
static INLINE bool _ACTIVE_ENC( const ENC_REF IENC );
static INLINE bool _WAITING_ENC( const ENC_REF IENC );
static INLINE bool _TERMINATED_ENC( const ENC_REF IENC );

static INLINE size_t  _GET_INDEX_PENC( const enc_t * PENC );
static INLINE ENC_REF _GET_REF_PENC( const enc_t * PENC );
static INLINE bool  _DEFINED_PENC( const enc_t * PENC );
static INLINE bool  _VALID_ENC_PTR( const enc_t * PENC );
static INLINE bool  _ALLOCATED_PENC( const enc_t * PENC );
static INLINE bool  _ACTIVE_PENC( const enc_t * PENC );
static INLINE bool  _TERMINATED_PENC( const enc_t * PENC );

static INLINE bool _INGAME_ENC_BASE( const ENC_REF IENC );
static INLINE bool _INGAME_PENC_BASE( const enc_t * PENC );

static INLINE bool _INGAME_ENC( const ENC_REF IENC );
static INLINE bool _INGAME_PENC( const enc_t * PENC );

//--------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------
static INLINE bool _VALID_ENC_RANGE( const ENC_REF IENC ) { return VALID_ENC_RANGE( IENC ); }
static INLINE bool _DEFINED_ENC( const ENC_REF IENC )     { return DEFINED_ENC( IENC );     }
static INLINE bool _ALLOCATED_ENC( const ENC_REF IENC )   { return ALLOCATED_ENC( IENC );   }
static INLINE bool _ACTIVE_ENC( const ENC_REF IENC )      { return ACTIVE_ENC( IENC );      }
static INLINE bool _WAITING_ENC( const ENC_REF IENC )     { return WAITING_ENC( IENC );     }
static INLINE bool _TERMINATED_ENC( const ENC_REF IENC )  { return TERMINATED_ENC( IENC );  }
...
static INLINE bool  _DEFINED_PENC( const enc_t * PENC )    { return _DEFINED_PENC( PENC );    }
static INLINE bool  _VALID_ENC_PTR( const enc_t * PENC )   { return _VALID_ENC_PTR( PENC );   }
static INLINE bool  _ALLOCATED_PENC( const enc_t * PENC )  { return _ALLOCATED_PENC( PENC );  }
static INLINE bool  _ACTIVE_PENC( const enc_t * PENC )     { return _ACTIVE_PENC( PENC );     }
static INLINE bool  _TERMINATED_PENC( const enc_t * PENC ) { return _TERMINATED_PENC( PENC ); }

static INLINE bool _INGAME_ENC_BASE( const ENC_REF IENC )  { return _INGAME_ENC_BASE( IENC );  }
static INLINE bool _INGAME_PENC_BASE( const enc_t * PENC ) { return _INGAME_PENC_BASE( PENC ); }

static INLINE bool _INGAME_ENC( const ENC_REF IENC )       { return _INGAME_ENC( IENC );  }
static INLINE bool _INGAME_PENC( const enc_t * PENC )      { return _INGAME_PENC( PENC ); }

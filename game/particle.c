...
void move_particles(void)
{
    ...
    while(cnt < MAXPRT)
    {
        if(prton[cnt])
        {
			...
			if( (prtsize[cnt]+prtsizeadd[cnt]) > 65536) prtsize[cnt] = 65536;
...
int load_one_particle(char *szLoadName, int object, int pip)
{
    ...
    if(fileread)
    {
        ...
        while(goto_colon_yesno(fileread))
        {
            ...
            test = ('I'-'A'<<15)|('D'-'A'<<10)|('A'-'A'<<5)|('M'-'A');  // [IDAM]
            if(idsz == test)  pipintdamagebonus[numpip] = iTmp;
            test = ('I'-'A'<<15)|('D'-'A'<<10)|('A'-'A'<<5)|('M'-'A');  // [WDAM]
...
void reset_particles(char* modname)
{
    ...
    numpip = 0;
	loadpath = "basicdat/globalparticles/1money.txt";
    if(load_one_particle(loadpath, 0, 0)==bfalse)
    {
        log_error("Data file was not found! (%s)", loadpath);
    }

	loadpath = "basicdat/globalparticles/5money.txt";
    if(load_one_particle(loadpath, 0, 0)==bfalse)
    {
        log_error("Data file was not found! (%s)", loadpath);
    }
	loadpath = "basicdat/globalparticles/25money.txt";
    if(load_one_particle(loadpath, 0, 0)==bfalse)
    {
        log_error("Data file was not found! (%s)", loadpath);
    }
	loadpath = "basicdat/globalparticles/100money.txt";
    if(load_one_particle(loadpath, 0, 0)==bfalse)
    {
        log_error("Data file was not found! (%s)", loadpath);
    }
	...
	//This is also global...
	loadpath = "basicdat/globalparticles/defend.txt";
    if(load_one_particle(loadpath, 0, 0)==bfalse)
    {
        log_error("Data file was not found! (%s)", loadpath);
    }

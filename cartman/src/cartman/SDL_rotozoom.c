...
int zoomSurfaceRGBA( SDL_Surface * src, SDL_Surface * dst, int smooth )
{
    ...
    */
    if (( sax = ( Uint32 * )calloc( sizeof( Uint32 ), ( dst->w + 1 ) ) ) == NULL )
    {
        return ( -1 );
    }
    if (( say = ( Uint32 * )calloc( sizeof( Uint32 ), ( dst->h + 1 ) ) ) == NULL )
    {
        free( sax );
        return ( -1 );
    }
...
int zoomSurfaceY( SDL_Surface * src, SDL_Surface * dst )
{
    ...
    * Allocate memory for row increments
    */
    if (( sax = ( Uint32 * )calloc( sizeof( Uint32 ), dst->w ) ) == NULL )
    {
        return ( -1 );
    }
    if (( say = ( Uint32 * )calloc( sizeof( Uint32 ), dst->h ) ) == NULL )
    {
        if ( sax != NULL )
        {
            free( sax );
                              ...
                              double *canglezoom, double *sanglezoom )
{
    ...
    *canglezoom *= zoom;
    ...
    sx = *sanglezoom * x;
    sy = *sanglezoom * y;
    dstwidthhalf = std::max(( int )
                       ceil( std::max( std::max( std::max( fabs( cx + sy ), fabs( cx - sy ) ), fabs( -cx + sy ) ), fabs( -cx - sy ) ) ), 1 );
    dstheighthalf = std::max(( int )
                        ceil( std::max( std::max( std::max( fabs( sx + cy ), fabs( sx - cy ) ), fabs( -sx + cy ) ), fabs( -sx - cy ) ) ), 1 );
